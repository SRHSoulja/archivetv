...
export function getCustomChannels() {
  try {
    ...
    return list.map((c) => {
      // Filter out any broken dummy placeholder items
      const validPrograms = (c.programs || []).filter(
        (p) => p && p.identifier !== 'classic_commercials_vol_1'
      );
      return {
        ...c,
...
export function saveCustomChannel(channel) {
  ...
  if (channel.id) {
    ...
    if (existingIdx !== -1) {
      ...
      updatedList[existingIdx] = { ...updatedList[existingIdx], ...channel };
      try {
        localStorage.setItem(CUSTOM_CHANNELS_KEY, JSON.stringify(updatedList));
      } catch {}
      return updatedList;
    }
  }
  ...
  const newChan = {
    id: channel.id || `custom_${Date.now()}`,
...
export function addProgramToChannel(channelId, programOrPrograms) {
  ...
  const newPrograms = Array.isArray(programOrPrograms) ? programOrPrograms : [programOrPrograms];
  const targetChannel = current[idx];
  const updatedPrograms = [...(targetChannel.programs || []), ...newPrograms];

  const updated = [...current];
  updated[idx] = { ...targetChannel, programs: updatedPrograms };
  try {
    localStorage.setItem(CUSTOM_CHANNELS_KEY, JSON.stringify(updated));
  } catch {}
  return updated;
...
export function reorderProgramsInChannel(channelId, fromIndex, toIndex) {
  ...
  const updated = [...current];
  updated[idx] = { ...targetChannel, programs };
  try {
    localStorage.setItem(CUSTOM_CHANNELS_KEY, JSON.stringify(updated));
  } catch {}
  return updated;
