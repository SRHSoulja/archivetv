...
const POSTER_CACHE_KEY = 'archivetv_poster_cache_v1';
...
function saveLocalPosterCache(key, url) {
  ...
  try {
    const obj = {};
    // Keep cache capped at 400 entries
...
export function cleanTitleForSearch(title) {
  ...
  return title
    .replace(/[_]/g, ' ')
    .replace(/\s*\([^)]*\)/g, '') // remove parenthesized years or notes
...
/**
 * Searches Wikipedia / Wikimedia Commons for the authentic theatrical poster / VHS cover art
 */
export async function fetchTheatricalPoster(title, year = '', identifier = '') {
  ...
  // 2. Check instant curated dictionary by identifier or sanitized title
  ...
  // 3. Query Wikipedia API with search + pageimages (pilicense=any for theatrical posters)
  const searchQueries = [
    year && year !== 'Vintage' ? `${clean} ${year} film` : `${clean} film`,
    clean,
  ];
  ...
  for (const q of searchQueries) {
    try {
      ...
      // Network failure or rate limit, proceed to next query or fallback
    }
  }
